    ...
    //wells and bands
      ...
      //wells
		...
		well_1
   			.datum([
   				[lMargin+bandW,Emin],
   				[lMargin+bandW,wellH],
   				[lMargin+bandW+wellW,wellH],
   				[lMargin+bandW+wellW,Emin]])
    		.attr('d',LineInEnergies);
    	well_2
   			.datum([
   				[lMargin+bandW+wellW+gapW,Emin],
   				[lMargin+bandW+wellW+gapW,wellH],
      ...
      //Fermi levels
    	...
    	function applyBias() {// apply a bias voltage
    			...
    			well_1
   					.datum([
   						...
   						[lMargin+bandW+wellW, E_from_lateralPercentage(lMargin+bandW+wellW,bias)],
   						[lMargin+bandW+wellW, Emin]])
    				.attr('d',LineInEnergies);
  				...
  				well_2
   					.datum([
   						[lMargin+bandW+wellW+gapW,Emin],
   						[lMargin+bandW+wellW+gapW,E_from_lateralPercentage(lMargin+bandW+wellW+gapW,bias)],
		...
		var zeroBias = function(){
    			...
    			well_1
   					.datum([
   						...
   						[lMargin+bandW+wellW, E_from_lateralPercentage(lMargin+bandW+wellW,0)],
   						[lMargin+bandW+wellW, Emin]])
   					.transition().ease('linear')
    				.attr('d',LineInEnergies);
  				...
  				well_2
   					.datum([
   						[lMargin+bandW+wellW+gapW,Emin],
   						[lMargin+bandW+wellW+gapW,E_from_lateralPercentage(lMargin+bandW+wellW+gapW,0)],
    	...
    	function E_from_lateralPercentage(x,bias){ //Energy value (vertical axis) from lateral position in percentage assuming a linear trend accross the tunneling gap
    		...
    		if (x > lMargin+bandW+wellW) {x -= gapW};
	...
	svgEnergies.append('rect')
        ...
        .attr('x',xEnScale(lMargin+bandW+wellW))
        .attr('y',0)
	...
	var occElectron = ELevels_gGate.selectAll('circle')
						...
						.append('circle')
						.attr('class','electron')
						.attr('cx',xEnScale(lMargin+bandW+wellW+gapW/2))
						.attr('cy',function(d){return yEnScale(d);})
						.attr('r',function(d){
							var actualEnergy = d + bias/2 - gateAtMol;
							return (actualEnergy<=Math.min(0,bias)) ? '10' : '0';
						});
    ...
    //labels
	    ...
	    svgEnergies.append('text')
			.attr("class", "axisLabel")
			.attr('x',xEnScale(lMargin+bandW+wellW+gapW/2))
	    	.attr('y',(h-hDiamond))
	    	.attr('dy','-2em')
	    	.text('MOLECULAR');
	    svgEnergies.append('text')
	    ...
	    svgEnergies.append('text')
			.attr("class", "axisLabel")
			.attr('x',xEnScale(lMargin+bandW+wellW/2))
	    	.attr('y',(h-hDiamond))
	    	.attr('dy','-2em')
	    	.text('vac.');
	    svgEnergies.append('text')
